<template>
  ...
  export default {
	...
	methods: {
	  register(){
		let success = axios.post("http://20.106.111.151/api/register", {
		"name": "test",
	  ...
	  async login(){
			...
			let success = await axios.post("http://20.106.111.151/api/login", loginCreds, {headers: { Accept: '*/*', "Content-Type":'application/json'}, withCredentials: true});
